...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return NextResponse.json({ error: 'ID de inventario no válido' }, { status: 400 });
    }
    ...
    for (const { productId, stock } of productsToAdd) {
      if (!mongoose.Types.ObjectId.isValid(productId)) {
        return NextResponse.json({ error: `ID de producto no válido: ${productId}` }, { status: 400 });
      }
      ...
      if (product) {
        ...
        return NextResponse.json({ error: `Producto con ID ${productId} no encontrado en el catálogo` }, { status: 404 });
      }
    }
    ...
    return NextResponse.json({
      message: 'Productos añadidos o actualizados en el inventario exitosamente',
      updatedProducts,
    }, { status: 201 });
  } catch (error) {
    console.error(error);
    return NextResponse.json({ error: 'Error al agregar productos al inventario', details: error }, { status: 500 });
  }
}
...
export async function GET(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const inventory = await Inventory.findById(id).populate('products.product_id');
    if (!inventory) {
      return NextResponse.json({ error: 'Inventario no encontrado' }, { status: 404 });
    }
    ...
    return NextResponse.json({ error: 'Error al obtener el inventario', details: error }, { status: 500 });
  }
}
...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ error: 'Error al actualizar el inventario', details: error }, { status: 500 });
  }
}
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ message: 'Inventario eliminado exitosamente' }, { status: 200 });
  } catch (error) {
    return NextResponse.json({ error: 'Error al eliminar el inventario', details: error }, { status: 500 });
  }
}
